 ...
 * https://fuelphp.com/forums/discussion/7283/functions-code-completion-on-netbeans-and-also-phpstorm
 *
 */
class Ccc
{
	...
	{
		// keep without-generated directory names
		...
		else
		{
			...
			if (\Cli::option('app', false) or \Cli::option('APP', false))
			{
				$dir = \File::read_dir(APPPATH, 0, 'php');
				self::generate_redefine($dir, '', APPPATH);
				$print_help = false;
			...
			if (\Cli::option('packages', false) or \Cli::option('PACKAGES', false))
			{
				$dir = \File::read_dir(PKGPATH, 0, 'php');
				self::generate_redefine($dir, '', PKGPATH);
				$print_help = false;
			...
			if ($print_help) // In case of no options or only unknown options (includes -help)
			{
				...
				echo 'If you want to work code completion on code used namespace, it is the better way switch comment off for the namespace statement during you edit it.'.PHP_EOL;
				exit(0);
			}
		}
