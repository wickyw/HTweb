...
class Utils {
	 ...
	 * @param string|null $redirect If empty, redirect back to last page
	 ...
	 * @param boolean $enforce_existence If true, when no session is associated report error
	 * @return mixed
	 * @throws HttpNotFoundException
	 */
	public static function valid_session(string $date, bool $enforce_existence=true) : ?\Sessions\Model_Session {
		...
		if($enforce_existence && empty($session)) {
	 ...
	 * @param int $user_id
	 * @param boolean $enforce_existence If true, when no user report error
	 * @return mixed
	 * @throws FuelException|HttpNotFoundException
	 */
	public static function valid_user(int $user_id, bool $enforce_existence=true) : ?Model_User {
		$user = Model_User::find($user_id);
		if ($enforce_existence && empty($user)) {
	 ...
	 * @param string|null $format A valid DateTime format string, defaults to DATETIME_FORMAT
	 * @return string
	 * @throws Exception
	 */
	 ...
	 * @param string|null $message A message to show when the model is empty.
	 ...
	 * @return string
	 */
	public static function get_short_head(int $length=7) : string {
		return substr(static::get_current_head(), 0, $length);
	}
}
